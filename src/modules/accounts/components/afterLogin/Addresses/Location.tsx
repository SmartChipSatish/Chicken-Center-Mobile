...
const Location = ({ handleAddress }: { handleAddress: (data: any) => void }) => {
  ...
  const isWithinHyderabadBounds = (lat: any, lon: any) => {
    return lat >= hyderabadBounds.minLat && lat <= hyderabadBounds.maxLat && lon >= hyderabadBounds.minLon && lon <= hyderabadBounds.maxLon;
  };

  const handleMapPress = async (e: any) => {
    const { latitude, longitude } = e.nativeEvent.coordinate;
    console.log(latitude, longitude)
    if (isWithinHyderabadBounds(latitude, longitude)) {
      setLatitude(latitude);
      setLongitude(longitude);
      try {
        const response = await Geocoder.from(latitude, longitude);
        ...
        if (city.toLowerCase() !== 'hyderabad') {
          setError('Only addresses in Hyderabad are allowed.');
          setLocation({ city: '', country: '', address: '', pincode: '', street: '', state: '' });
        } else {
          const countryComponent = addressComponents.find(component =>
            component.types.includes('country')
      ...
      setError('Please select a location within Hyderabad.');
    }
  };
